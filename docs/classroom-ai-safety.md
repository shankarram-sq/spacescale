...
it afterwards. That adoption is the part worth watching: it changes what a later
Section move carries, so the result reports how many objects were taken in
...
nothing when the note already has the size asked for. Growing or shrinking
across a Section's edge changes membership through the same path a participant's
own drag-resize uses. Like a move, it leaves the note's author and text alone
