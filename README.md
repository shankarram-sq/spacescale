...
covers the location it names, or to the one object selected in that browser, and may carry
one picture or one public video with its text—the same material the canvas takes, through
the same asset pipeline and the same link check, so a reply can show the diagram or the clip
it is talking about right beside the work. Everything the
